...
export const getPosts = async (last = -1) => {
  ...
  let querySnapshot = query(ref, orderBy('updatedAt', 'desc'), limit(LIMIT));
...
export const getMoreUserPublishedPosts = async (uid, last = -1) => {
...
export const getSinglePostAndPath = async (userUid, slug) => {
  const ref = doc(db, 'users', userUid, 'posts', slug);
  const querySnapshot = await getDoc(ref);
  return { post: querySnapshot, path: ref.path };
};
