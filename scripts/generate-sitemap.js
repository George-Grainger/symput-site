...
async function generateSitemap() {
  ...
  const urlSet = pages
    .map((page) => {
      ...
      const priority = ['/index', '/feedback'].includes(path) ? '1.0' : '0.5';
      // Build url portion of sitemap.xml
      return `<url><loc>https://www.symput.com${route}</loc><priority>${priority}</priority><changefreq>daily</changefreq></url><url><loc>https://www.symput.com/ar${route}</loc><priority>${priority}</priority><changefreq>daily</changefreq></url><url><loc>https://www.symput.com/zh-cn${route}</loc><priority>${priority}</priority><changefreq>daily</changefreq></url>`;
    })
    .join('');
